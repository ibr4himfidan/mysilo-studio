...
test('local request origin follows the loopback host without trusting forwarded host',()=>{
  const previous=process.env.MYSILO_PUBLIC_ORIGIN;delete process.env.MYSILO_PUBLIC_ORIGIN;
  try{const request=publicRequest(new Request('http://localhost:5188/api/auth/login',{method:'POST',headers:{host:'127.0.0.1:5188',origin:'http://127.0.0.1:5188','content-type':'application/json','x-forwarded-host':'attacker.example'},body:'{}'}));sameOrigin(request);assert.equal(new URL(request.url).host,'127.0.0.1:5188')}finally{if(previous!==undefined)process.env.MYSILO_PUBLIC_ORIGIN=previous}
});
